	...
	if not self.Locked then
		table.insert(tab, {
			...
			Callback = function()
				...
				self.Knife = math.random(0, 1) == 1
				self.Chestplate = math.random(0, 9)
				self.ShoulderLeft = math.random(0, 13)
				self.ShoulderRight = math.random(0, 13)
				self.Wrist = math.random(0, 5)
				self.KneepadRight = math.random(0, 5)
				self.KneepadLeft = math.random(0, 5)
		...
		table.insert(tab, {
			...
			Callback = function(val)
				self.Chestplate = math.Clamp(math.Round(val), 0, 9)
		...
		table.insert(tab, {
			...
			Callback = function(val)
				self.ShoulderLeft = math.Clamp(math.Round(val), 0, 13)
		...
		table.insert(tab, {
			...
			Callback = function(val)
				self.ShoulderRight = math.Clamp(math.Round(val), 0, 13)
		...
		table.insert(tab, {
			Name = "Set right Knee",
			Client = function()
				return GAMEMODE:OpenGUI("Input", "number", "Set right Knee", {
					Default = self.KneepadRight or 0
				})
			end,
			Callback = function(val)
				self.KneepadRight = math.Clamp(math.Round(val), 0, 5)
		...
		table.insert(tab, {
			Name = "Set left knee",
			Client = function()
				return GAMEMODE:OpenGUI("Input", "number", "Set left knee", {
					Default = self.KneepadLeft or 0
				})
			end,
			Callback = function(val)
				self.KneepadLeft = math.Clamp(math.Round(val), 0, 5)
	...
	function ITEM:GetModelData(ply)
		return {
			_base = {
				...
				Bodygroups = {
					Helmet_Attachments = self.HelmetAttachment,
					Chest = self.Chestplate,
					Knife = self.Knife and 1 or 0,
					Left_Shoulder = self.ShoulderLeft,
					Right_Shoulder = self.ShoulderRight,
					Wrist = self.Wrist,
					Kneepad_Right = self.KneepadRight,
					Kneepad_Left = self.KneepadLeft,
					Arms = self.Arm and 1 or 0
				}
			}
		}
	end
